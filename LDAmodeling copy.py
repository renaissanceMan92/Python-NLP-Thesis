...
#Building the model.    
def create_topic_model(dictionary,corpus):
...
dictionary_post_covid,corpus_post_covid,data_ready_post_covid=get_data(DataTypes.POST_COVID_DATA)
dictionary_pre_covid,corpus_pre_covid,data_ready_pre_covid=get_data(DataTypes.PRE_COVID_DATA)

lda_model_post_covid = create_topic_model(dictionary_post_covid,corpus_post_covid)       
lda_model_pre_covid = create_topic_model(dictionary_pre_covid,corpus_pre_covid)       
...
# Show
text_file = open("Output/most_representative_document.html", "w",encoding='UTF-8')
...
#Save top topics
top_topics = open('Output/top_topics.txt','w',encoding='UTF-8')
